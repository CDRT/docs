![Commercial Deployment Readiness Team](../../../img/cdrt.png)

[Home](/)

[**BIOS Reference**](bios/bios_top.md)

> [**Settings Dictionary**](bios/settings/dictionary.md)

<a href="/#/bios/settings/thinkpad/config.md" style="display:contents;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThinkPad</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href="/#/bios/settings/thinkcentre/main.md" style="display:contents;">ThinkCentre</a>

---

>> ***ThinkStation***

---

- [Main](bios/settings/thinkstation/main.md)
   - [System Summary](bios/settings/thinkstation/systemsummary.md)
   - [System Time & Date](bios/settings/thinkstation/systemtimedate.md)
   - [BIOS Event Log](bios/settings/thinkstation/bioseventlog.md)
- [Devices](bios/settings/thinkstation/devices.md)
   - [Serial Port Setup](bios/settings/thinkstation/serialportsetup.md)
   - [Parallel Port Setup](bios/settings/thinkstation/parallelportsetup.md)
   - [USB Setup](bios/settings/thinkstation/usbsetup.md)
   - [Storage Setup](bios/settings/thinkstation/storagesetup.md)
		- [Intel(R) VMD Technology](bios/settings/thinkstation/intelvmd.md)
   - [ATA Drive Setup](bios/settings/thinkstation/atadrivesetup.md)
   - [NVME Drive Setup](bios/settings/thinkstation/nvme.md)
   - [Video Setup](bios/settings/thinkstation/videosetup.md)
   - [Audio Setup](bios/settings/thinkstation/audiosetup.md)
   - [Network Setup](bios/settings/thinkstation/networksetup.md)
   - [PCIe Setup](bios/settings/thinkstation/pciesetup.md)
   - [PCI Express Setup](bios/settings/thinkstation/pciexpress.md)
   - [Intel Thunderbolt](bios/settings/thinkstation/intelthunderbolt.md)
   - [BMC Setup](bios/settings/thinkstation/bmcsetup.md)
- [Advanced](bios/settings/thinkstation/advanced.md)
   - [Diagnostics (AMD)](bios/settings/thinkstation/diagnostics.md)
   - [PCIe Configuration (AMD)](bios/settings/thinkstation/pcieconfiguration.md)
   - [PCI Express Setup](bios/settings/thinkstation/pciexpressconfig.md)
   - [CPU Configuration (AMD)](bios/settings/thinkstation/cpuconfiguration.md)
   - [CPU Setup (Intel)](bios/settings/thinkstation/cpusetup.md)
   - [Memory Configuration](bios/settings/thinkstation/memoryconfig.md)
   - [RAS Configuration (AMD)](bios/settings/thinkstation/rasconfiguration.md)
   - [Common RefCode Configuration (AMD)](bios/settings/thinkstation/commonrefcodeconfiguration.md)
   - [DASH Configuration (AMD)](bios/settings/thinkstation/dashconfiguration.md)
   - [Intel Manageability Settings](bios/settings/thinkstation/intelmanageability.md)
   - [Serial Port Console Redirection (AMD)](bios/settings/thinkstation/serialportconsoleredirection.md)
- [Power](bios/settings/thinkstation/power.md)
   - [Automatic Power On](bios/settings/thinkstation/autopoweron.md)
- [Security](bios/settings/thinkstation/security.md)
   - [Hard Disk Password](bios/settings/thinkstation/harddiskpass.md)
   - [Password Policies](bios/settings/thinkstation/passwordpolicies.md)
   - [SMP Access Control](bios/settings/thinkstation/smpaccesscontrol.md)
   - [TCG Feature Setup](bios/settings/thinkstation/tcgfeaturesetup.md)
   - [Secure Boot](bios/settings/thinkstation/secureboot.md)
   - [System Event Log](bios/settings/thinkstation/systemeventlog.md)
   - [Certificate-based BIOS Authentication](bios/settings/thinkstation/certbasedbiosauth.md)
- [Startup](bios/settings/thinkstation/startup.md)
   - [Boot Priority Order](bios/settings/thinkstation/bootpriorityorder.md)
- [Exit](bios/settings/thinkstation/exit.md)
