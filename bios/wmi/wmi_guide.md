...
BIOS settings, restore settings to their factory defaults, change single settings, change passwords, and
modify the boot order either directly on client computers or remotely.

If you have suggestions, comments, or questions, please talk to us on our forum! A team of deployment engineers
is standing by, ready to help with any deployment challenges you are facing: [Enterprise Client Management Forum](https://forums.lenovo.com/t5/Enterprise-Client-Management/bd-p/sa01_eg)
...
<img src="../img/guides/bios/biosWMI.png" alt="" style="text-align:center;" />
...
To support these complex BIOS passwords, a new WMI class is required: Lenovo_WmiOpcodeInterface class. If you don’t need complex passwords, you can continue using the same WMI classes and methods that you have always used. You only need to change
WMI classes and methods if you want to use complex BIOS passwords

### Set/Save a Setting

In previous ThinkPad models, it was required to specify the supervisor password in both Lenovo_SetBiosSetting and Lenovo_SaveBiosSettings. This is no longer required. Instead, the supervisor password is specified using a new class, Lenovo_WmiOpcodeInterface. The new order in which method calls are made is:

1. Lenovo_SetBiosSetting
1. Lenovo_WmiOpcodeInterface
1. Lenovo_SaveBiosSettings

### Specifying the BIOS Supervisor Password

In previous ThinkPad models, the BIOS supervisor password was specified like “mypassword,ascii,us”. In new ThinkPad models, specify the BIOS supervisor password like “WmiOpcodePasswordAdmin:MyPassword” using the Lenovo_WmiOpcodeInterface class.
...
For 2018 and newer models, the password is specified using the Lenovo_WmiOpcodeInterface class. A single parameter in the format of “WmiOpcodePasswordAdmin:MyPassword” (where MyPassword is your actual password) is used.

For 2017 and older models, if a supervisor password is already set, you must specify that supervisor password before you can change any BIOS settings. The format for password parameters is:

``` <password>, [ascii|scancode], [us|fr|gr] ```

 The parameters are described in the table below. For Example, if the Current password is “abc”, Password encoding is “ascii”, Keyboard language is “us”, Lenovo_SaveBiosSettings should be called with a parameter string = “abc, ascii, us;”

Table 4 described the password parameters for old models that produced before 2017.
Table 5 described the password parameters for 2018 or newer models.
...
For 2018 or newer computers, you need first call Class Lenovo_WmiOpcodeInterface with below parametes before change any BIOS
...
_**Table 5.** 2018 and later password parameters format, password authentication_
</div>
...
1. Identify the value to which the setting will be changed, using the Lenovo_GetBiosSelections class.

1. Change the BIOS setting to the desired value using the Lenovo_SetBiosSetting class, then use the Lenovo_SaveBiosSetting class to save the settings.
...
?> 	**1**. “Boot Order” settings are case sensitive.<br>
	**2**. Settings may differ among products, it is recommended to list all settings first to identify the BIOS setting you want to change, and put all the listed devices into the script parameters in the desired order. <br>
...
To change a password, specify a password type and format of the password. The format for the password parameter string is:

```"[password type], [current password], [new password], [encoding], [keyboard language]"```
...
Each parameter uses a different key word and requires a separate call to the Lenovo_WmiOpcodeInterface method. The parameter ”WmiOpcodePasswordSetUpdate;” MUST be the LAST call to the Lenovo_WmiOpcodeInterface method to make the change take effect.

The parameters are described in Table 8, the supported characters are listed in Table 9.
...
1. A password cannot be set using this method when one does not already exist. Passwords can only be changed or cleared.

1. To remove the power-on password when a supervisor password is set, it must be done in three steps total:
   ...
   1. Change the power-on password by specifying the current password and a NULL string as the new password
   1. Reboot (do not reboot between steps A and B)

1. Some security-related settings can only be disabled when a Supervisor password exists. For example, the following BIOS settings cannot be changed from Enable to Disable unless you have a Supervisor password:
...
1. Set an authentication level of "pktPrivacy"

See the sample Visual Basic scripts for configuring BIOS settings below to see how to implement WMI-based administration scripts that include these parameters for encryption.
...
Sample scripts referenced below are provided on [Lenovo’s support website](https://support.lenovo.com/us/en/documents/ht100612)
...
1. Specify the supervisor password (if it exists).
